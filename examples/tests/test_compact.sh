...
if [ "${R_CLUSTER_IDS}" != "VARIABLE IS NOT DEFINED!" ]; then
  ...
  for cluster_id in ${CLUSTER_IDS}; do
    ansible-playbook compact_kubevirt.yaml -e action='delete' -e cluster_id=${cluster_id} -e cluster_name=${TESTNAME} -e ocp_namespace=test-${TESTNAME}
  done
fi
...
ansible-playbook compact_kubevirt.yaml -e cluster_name=${TESTNAME} -e ocp_namespace=test-${TESTNAME} -e overwrite_dns=true
